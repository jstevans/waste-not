...
export default function configure(
  ...
  options: Options,
  overrides?: Overrides
): DependencyGetter<string> {
  return function getDependenciesForFile(
    filePath: string,
    code?: string
  ): MaybeDependencies {
    try {
      ...
      const { parse = defaultParser, resolver = defaultResolver } =
        overrides || {};
      ...
      let fileDepFailures = fileDependenciesWithFailures
        ...
        .map(
          (dep) =>
            `Failed to resolve dependency '${dep[0]}' in file '${filePath}'`
        );

      warnings = [...warnings, ...fileDepFailures];

      fileDependencies = fileDependenciesWithFailures
        .filter((dep) => dep[1] && dep[1].length > 0)
        .map((dep) => dep[1]);

      let wildcardAliasDependencies = wildcardDependencies.map((dep) =>
        getWildcardPathAliases(dep, filePath, options.baseDirectory, tsConfig)
      );

      let wildcardFileDependencies = wildcardAliasDependencies
        .map((wad) =>
          getMatchedStrings(Object.keys(allFiles), [
            ...wad.original,
            ...wad.aliases,
          ])
        )
